...
BROWSE_TARGETS = {
    "📦 Все объявления": ("all", None, "📦 Все объявления"),
    "🎁 Бесплатно": ("free", None, "🎁 Бесплатно"),
    "💸 За копейки": ("cheap", None, "💸 За копейки"),
    "👕 Одежда": ("all", "Одежда", "👕 Одежда"),
    "👟 Обувь": ("all", "Обувь", "👟 Обувь"),
    "📱 Электроника": ("all", "Электроника", "📱 Электроника"),
    "🧸 Детям": ("all", "Детям", "🧸 Детям"),
    "🏡 Дом и дача": ("all", "Дом и дача", "🏡 Дом и дача"),
    "🚗 Авто": ("all", "Авто", "🚗 Авто"),
}
...
def get_user_profile_text(chat_id: int) -> str:
    ...
    return (
        ...
        f"❤️ В избранном: {len(get_favorites(chat_id))}\n"
...
def submenu_menu():
    ...
    kb.row("🏠 Главное меню", "🎁 Пригласить друга")
    return kb
...
def build_card_keyboard(item_id: int, viewer_tg: int, owner_tg: int):
    ...
    kb.row(
        types.InlineKeyboardButton("🗑️ Удалить", callback_data=f"delete_{item_id}"),
        types.InlineKeyboardButton("➡️ Далее", callback_data="next_item")
    )
    kb.row(
...
def back(message):
    ...
    if mode in ["browse_all", "browse_free", "browse_cheap", "filtered"] or str(mode).startswith("browse_cat:"):
        set_view_state(chat_id, 0, mode="browse_menu", photo_idx=0)
        bot.send_message(
            chat_id,
            "Выберите категорию или смотрите все объявления",
...
def callback_handler(call):
    ...
    if data.startswith("prev_"):
        try:
            current_item_id = int(data.split("_")[1])
    ...
    if data.startswith("share_"):
        ...
        bot.send_message(
            chat_id,
            build_share_text(item_id)
        )

        bot.answer_callback_query(call.id, "Текст для отправки готов")
        return
