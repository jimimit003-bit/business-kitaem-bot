...
def watch_items(message):
    ...
    if not items:
        bot.send_message(chat_id, "Объявлений нет 😔", reply_markup=main_menu())
        return
...
def favorites_menu(message):
    ...
    if not fav_ids:
        bot.send_message(chat_id, "В избранном пусто", reply_markup=main_menu())
        return
...
def report_callback(call):
    ...
    bot.send_message(
        ...
        reply_markup=report_reason_kb(item_id)
    )
...
def report_reason_callback(call):
    ...
    add_report(call.message.chat.id, item_id, reason)

    bot.answer_callback_query(call.id, "Жалоба отправлена")
    bot.send_message(call.message.chat.id, "Спасибо. Жалоба сохранена.")


@bot.callback_query_handler(func=lambda call: call.data == "reportcancel")
...
def callback_handler(call):
    ...
    if data.startswith("share_"):
        item_id = int(data.split("_")[1])
        bot.send_message(chat_id, build_share_text(item_id))
        bot.answer_callback_query(call.id)
        return


    if data.startswith("take_"):
        bot.answer_callback_query(call.id, "Напиши владельцу 💬")
        return
    ...
    if data.startswith("taken_"):
        ...
        if mark_taken(item_id, chat_id):
            bot.answer_callback_query(call.id, "Отдано")
        else:
            bot.answer_callback_query(call.id, "Ошибка")
    ...
    if data.startswith("bump_"):
        ...
        new_id = bump_item(item_id, chat_id)

        if new_id:
            bot.answer_callback_query(call.id, "Поднято 🚀")
        else:
            bot.answer_callback_query(call.id, "Ошибка")
...
# CREATE FLOW
# =========================
@bot.message_handler(func=lambda m: m.chat.id in pending_create)
def create_flow(message):
    chat_id = message.chat.id
    flow = pending_create[chat_id]

    if flow["step"] == "title":
        flow["data"]["title"] = message.text
        flow["step"] = "price"
        bot.send_message(chat_id, "Цена? Напиши 0 если бесплатно")
        return

    if flow["step"] == "price":
        try:
            flow["data"]["price"] = int(message.text)
        except:
            bot.send_message(chat_id, "Цена должна быть числом")
            return

        flow["step"] = "city"
        bot.send_message(chat_id, "Город?")
        return

    if flow["step"] == "city":
        flow["data"]["city"] = message.text
        flow["step"] = "category"
        bot.send_message(chat_id, "Категория?", reply_markup=category_pick_kb())
        return

    if flow["step"] == "category":
        if message.text not in CATEGORIES:
            bot.send_message(chat_id, "Выбери категорию кнопкой")
            return

        flow["data"]["category"] = message.text
        flow["step"] = "photo"

        bot.send_message(
            chat_id,
            "Отправь фото (до 5) или нажми «Готово без фото»",
...
def photo_handler(message):
    ...
    photos = flow["data"]["photos"]

    if len(photos) >= MAX_PHOTOS_PER_ITEM:
        bot.send_message(chat_id, "Максимум 5 фото")
        return
...
@bot.message_handler(func=lambda m: m.text in ["✅ Готово без фото", "✅ Опубликовать"])
def finish_create_handler(message):
    chat_id = message.chat.id
